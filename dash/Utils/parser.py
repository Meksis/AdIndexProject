...
def get_post_data(post_card_object, driver):
    ...
    return(pd.DataFrame(
            [
                {
                'date' : publicated_at,
                # 'author' : random.choice(authors_names) if not author_name else author_name,
                'author' : 'Не указан' if not author_name else author_name,
                'post_name' : post_name,
                'post_tag' : tag,
                'post_id' : id,
                'link' : link
...
def parse_some_news(date_from : datetime.date, date_to : datetime.date, driver : webdriver.Chrome = None, next_page_delay: float = 1) -> pd.DataFrame:
    ...
    while continue_search:
        ...
        for post_data in posts_cards[checked_rounds * 16 :]:
            parsed_data, post_date = get_post_data(post_data, driver)
